...
class OpenId {
	const COOKIE_EXPIRE_TIME = 8640000;
	const COOKIE_ID = '__openid';
	const PRIVATE_KEY = 'f!~183a';
	const USER_EMAIL_SPLITTER = '_X_X';

	/*
	 * 通过Google公共帐号登录，获取用户的email
	...
	public static function getUserEmail() {
		...
		if(!$openid->mode) {
			...
			die;
		} elseif ($openid->mode != 'cancel' //确保用户没有取消认证
			&& $openid->validate()) {
	...
	{
		$str = mcrypt_decrypt(MCRYPT_DES, self::PRIVATE_KEY, $str, MCRYPT_MODE_ECB);
...
class LightOpenID
{
	...
	function __construct($host)
	{
		...
		$uri = rtrim(preg_replace('#((?<=\?)|&)openid[\._][^&]+#', '', $_SERVER['REQUEST_URI']), '?');
